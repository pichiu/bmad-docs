...
```mermaid
flowchart TD
    A["用戶選擇 [1] User-Selected"] --> B[讀取 brain-methods.csv]
    B --> C[解析欄位]
...
**技術概念說明：惰性載入（Lazy Loading）**

這種按需載入策略是**惰性載入模式**的應用：

```mermaid
sequenceDiagram
    participant User as 用戶
    participant Step1 as Step 1
    participant Step2a as Step 2a
    participant CSV as brain-methods.csv

    Note over Step1,CSV: 惰性載入策略
    User->>Step2a: 選擇 User-Selected
    Note over Step2a: 此時才載入 CSV
    Step2a->>CSV: 讀取技術資料
    CSV-->>Step2a: 返回 62 技術
    Step2a->>User: 呈現類別清單
```
...
flowchart TD
    ...
    B -->|選擇技術| C[技術加入選擇清單]
    B -->|Details| D[顯示技術完整說明]
    B -->|Categories| E[回到類別瀏覽頁面]
...
**技術概念說明：漸進式揭露（Progressive Disclosure）**
...
graph TD
    ...
    subgraph "第二層：類別內容"
        B1["5-11 個技術<br/>名稱 + 簡介"]
    end

    subgraph "第三層：詳細資訊"
        C1["完整說明<br/>能量等級<br/>適用場景<br/>範例 Prompt"]
    end
...
| 模式名稱 | 應用位置 | 核心價值 |
|----------|----------|----------|
| **Lazy Loading** | CSV 技術載入 | 效能優化 |
| **Progressive Disclosure** | 三層資訊架構 | 降低認知負荷 |
| **Librarian Pattern** | AI 角色定位 | 保持中立客觀 |
| **Confirmation Loop** | 選擇確認 | 防止誤選 |
