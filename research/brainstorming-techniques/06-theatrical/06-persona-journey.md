...
| 相關模式/方法論 | 連結點 | 應用價值 |
|----------------|--------|----------|
| **Method Acting** | Stanislavski體系 | 完全投入角色的表演訓練方法 |
| **Empathy Mapping** | 設計思維工具 | 系統化理解用戶所想所感所做 |
| **Customer Journey Mapping** | 服務設計 | 視覺化完整用戶旅程的標準方法 |
| **Jobs to Be Done** | Christensen創新理論 | 理解用戶「雇用」產品的真正任務 |
| **Narrative Inquiry** | 敘事研究法 | 透過故事理解深層經驗與意義 |
| **Embodied Cognition** | 具身認知理論 | 身體參與增強理解與記憶 |

**核心洞察**：Persona Journey超越傳統Persona的關鍵在於「從描述到體驗」的躍遷。閱讀一份用戶檔案是「關於」用戶的知識，而「成為」用戶五分鐘獲得的是「作為」用戶的智慧——這兩者有本質差異。Method Acting演員知道：當你真正活進一個角色，你會發現劇本上沒寫的東西。同樣地，當設計師真正活進用戶的一天，會發現傳統用戶研究問不出的需求。最深的洞察往往不是用戶「說」的，而是用戶「是」的——而這種存在層面的理解，只有透過體驗式同理心才能觸及。
