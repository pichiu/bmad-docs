...
| 相關模式/方法論 | 連結點 | 應用價值 |
|----------------|--------|----------|
| **MCDA (Multi-Criteria Decision Analysis)** | 學術基礎 | 多準則決策的理論框架 |
| **Pugh Matrix** | 相對比較版 | 用基準方案進行相對評估 |
| **AHP (Analytic Hierarchy Process)** | 層級分析 | 更嚴謹的權重設定方法 |
| **Weighted Scoring Model** | 加權評分 | 企業常用的專案評估法 |
| **Cost-Benefit Analysis** | 成本效益分析 | 財務導向的決策方法 |

**核心洞察**：解決方案矩陣最危險的陷阱是「假客觀」——用數字包裝主觀判斷，然後假裝這是科學。矩陣的真正價值不是產出「7.3分 vs 7.1分」的精確排名，而是逼迫團隊回答：「我們最在意什麼？」（權重討論）和「這個方案真的在這方面更好嗎？」（評分討論）。最好的矩陣使用者會在結果出來後問：「這感覺對嗎？」如果直覺說不對，那就是有假設需要重新檢視——而不是盲目接受數字。數字是對話的起點，不是終點。
