...
| 模式名稱 | 應用位置 | 核心價值 |
|----------|----------|----------|
| **Cross-Domain Transfer** | 類比構建 | 從遠距領域借用解決方案 |
| **Pattern Matching** | 結構識別 | 識別不同表面下的相同結構 |
| **Surface vs Structure** | 分析深度 | 區分表面相似和結構相似 |
| **Near vs Far Analogy** | 創意強度 | 距離越遠，創新潛力越大 |

**核心洞察**：類比思維的關鍵是「抽象層次」的選擇。表面類比（「這和那個很像」）通常不產生新洞見；結構類比（「這背後的原理和那個相同」）才是創新的來源。Velcro 的發明者沒有看到植物刺「像」什麼，他看到的是「一種連接機制」——這個抽象讓他能夠在完全不同的材料領域應用同樣的原理。
